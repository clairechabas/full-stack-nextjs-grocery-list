...
export const CardHeader: NextPage<CardHeaderProps> = ({
  ...
  return (
    <div className='flex flex-row items-center justify-between p-3 border-b border-slate-200'>
      ...
      <button className='text-sm font-medium text-gray-600 underline' type='button' onClick={clearAllFn}>
        Clear all
      </button>
    </div>
  )
...
export const List: NextPage<CardProps> = ({ children }) => {
  return (
    <div className='overflow-y-auto h-72'>{ children }</div>
  )
}
...
export const ListItemComponent: NextPage<ListItemProps> = ({ item, onUpdate }) => {
  return (
